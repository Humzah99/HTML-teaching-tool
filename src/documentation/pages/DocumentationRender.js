...
const DocumentationRender = () => {
  ...
  const activeDocument = DUMMY_DOCUMENTATION_DATA.find(
    (d) => d.id === documentationId
  );
    ...
    <>
      ...
      <div className="container mb-5">
        <div className="documentation-title mt-4">
          <h2>HTML {activeDocument.title}</h2>
        </div>
        <div className="hr-line mb-2"></div>
        {activeDocument.content.map((information, index) => {
          return (
            <div className="tutorial-section" key={index}>
              <h4 className="mb-2">{information.subTitle}</h4>
              {activeDocument.content[index].information.map(
                (information, index) => {
                  return (
                    <p
              ...
              {activeDocument.content[index].codeString != null && (
                <>
                  <SyntaxHighlighter
                    language="javascript"
                    style={duotoneForest}
        ...
        {activeDocument.chapterSummary.length > 0 && (
          <>
            <div className="chapter-summary-title mb-3">
              <h4>Chapter Summary</h4>
            </div>
              {activeDocument.chapterSummary.map((summary, index) => {
                return (
                  <p
